...
//
//
//  Created by 程倩 on 15/9/1.
//  Copyright © 2015年 chengqian. All rights reserved.
//
...
public enum CQ_AlignType {
    ...
    private func layoutAttributes(isInner: Bool, isVertical: Bool) -> CQ_LayoutAttributes {
        let attributes = CQ_LayoutAttributes()
        ...
        switch self {
            ...
            // 仅内部参照需要
            case .Center:
                return CQ_LayoutAttributes(horizontal: .CenterX, referHorizontal: .CenterX, vertical: .CenterY, referVertical: .CenterY)
        }
    }
}
...
extension UIView {
    ...
    :returns: 约束数组
    */
    public func cq_Fill(referView: UIView, insets: UIEdgeInsets = UIEdgeInsetsZero) -> [NSLayoutConstraint] {
    ...
    :returns: 约束数组
    */
    public func cq_AlignInner(type type: CQ_AlignType, referView: UIView, size: CGSize?, offset: CGPoint = CGPointZero) -> [NSLayoutConstraint]  {
        
        return cq_AlignLayout(referView, attributes: type.layoutAttributes(true, isVertical: true), size: size, offset: offset)
    }
    ...
    :returns: 约束数组
    */
    public func cq_AlignVertical(type type: CQ_AlignType, referView: UIView, size: CGSize?, offset: CGPoint = CGPointZero) -> [NSLayoutConstraint] {
        
        return cq_AlignLayout(referView, attributes: type.layoutAttributes(false, isVertical: true), size: size, offset: offset)
    }
    ...
    :returns: 约束数组
    */
    public func cq_AlignHorizontal(type type: CQ_AlignType, referView: UIView, size: CGSize?, offset: CGPoint = CGPointZero) -> [NSLayoutConstraint] {
        
        return cq_AlignLayout(referView, attributes: type.layoutAttributes(false, isVertical: false), size: size, offset: offset)
    }
    ...
    :returns: 约束数组
    */
    public func cq_HorizontalTile(views: [UIView], insets: UIEdgeInsets) -> [NSLayoutConstraint] {
        ...
        firstView.cq_AlignInner(type: CQ_AlignType.TopLeft, referView: self, size: nil, offset: CGPoint(x: insets.left, y: insets.top))
        ...
        for i in 1..<views.count {
            let subView = views[i]
            cons += subView.cq_sizeConstraints(firstView)
            subView.cq_AlignHorizontal(type: CQ_AlignType.TopRight, referView: preView, size: nil, offset: CGPoint(x: insets.right, y: 0))
            preView = subView
        }
    ...
    :returns: 约束数组
    */
    public func cq_VerticalTile(views: [UIView], insets: UIEdgeInsets) -> [NSLayoutConstraint] {
        ...
        for i in 1..<views.count {
            let subView = views[i]
            cons += subView.cq_sizeConstraints(firstView)
            subView.cq_AlignVertical(type: CQ_AlignType.BottomLeft, referView: preView, size: nil, offset: CGPoint(x: 0, y: insets.bottom))
            preView = subView
        }
    ...
    :returns: 对应的约束
    */
    public func cq_Constraint(constraintsList: [NSLayoutConstraint], attribute: NSLayoutAttribute) -> NSLayoutConstraint? {
    ...
    :returns: 约束数组
    */
    private func cq_AlignLayout(referView: UIView, attributes: CQ_LayoutAttributes, size: CGSize?, offset: CGPoint) -> [NSLayoutConstraint] {
        
        translatesAutoresizingMaskIntoConstraints = false
        
        var cons = [NSLayoutConstraint]()
        
        cons += cq_positionConstraints(referView, attributes: attributes, offset: offset)
        
        if size != nil {
            cons += cq_sizeConstraints(size!)
        }
    ...
    :returns: 约束数组
    */
    private func cq_sizeConstraints(size: CGSize) -> [NSLayoutConstraint] {
    ...
    :returns: 约束数组
    */
    private func cq_sizeConstraints(referView: UIView) -> [NSLayoutConstraint] {
    ...
    :returns: 约束数组
    */
    private func cq_positionConstraints(referView: UIView, attributes: CQ_LayoutAttributes, offset: CGPoint) -> [NSLayoutConstraint] {
...
///  布局属性
private final class CQ_LayoutAttributes {
